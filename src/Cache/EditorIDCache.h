...
class EditorID : public Singleton<EditorID> {
 public:
  void CacheEditorID(RE::FormID formID, const char* editorID) {
    Locker locker(lock_);
    formIDToEditorIDMap_.set_get(formID, editorID);
  ...
  std::string GetEditorID(RE::FormID formID) {
    return formIDToEditorIDMap_.set_get(formID, "");
  }
 ...
 private:
  ...
  mutable Lock lock_;
  emhash5::HashMap<RE::FormID, std::string> formIDToEditorIDMap_;
