...
// Used to interact with the input method editor (IME). Users of MinGW should
// manually link to this.
#ifdef _MSC_VER
...
void SystemInterface_Win32::SetClipboardText(const Rml::String& text_utf8) {
  if (window_handle) {
    if (!OpenClipboard(window_handle)) return;
...
void SystemInterface_Win32::GetClipboardText(Rml::String& text) {
  if (window_handle) {
    ...
    if (clipboard_text) text = RmlWin32::ConvertToUTF8(clipboard_text);
...
void SystemInterface_Win32::ActivateKeyboard(Rml::Vector2f caret_position,
                                             float line_height) {
...
bool RmlWin32::WindowProcedure(
    Rml::Context* context,
    TextInputMethodEditor_Win32& text_input_method_editor, HWND window_handle,
    UINT message, WPARAM w_param, LPARAM l_param) {
  if (!context) return true;

  static bool tracking_mouse_leave = false;

  // If the user tries to interact with the window by using the mouse in any
  // way, end the composition by committing the current string. This behavior is
  // identical to other browsers and is expected, yet, Windows does not send any
  // IME messages in such a case.
  if (text_input_method_editor.IsComposing() && message >= WM_LBUTTONDOWN &&
  ...
  switch (message) {
    case WM_LBUTTONDOWN:
      result =
          context->ProcessMouseButtonDown(0, RmlWin32::GetKeyModifierState());
      SetCapture(window_handle);
      break;
    case WM_LBUTTONUP:
      ReleaseCapture();
      result =
          context->ProcessMouseButtonUp(0, RmlWin32::GetKeyModifierState());
      break;
    case WM_RBUTTONDOWN:
      result =
          context->ProcessMouseButtonDown(1, RmlWin32::GetKeyModifierState());
      break;
    case WM_RBUTTONUP:
      result =
          context->ProcessMouseButtonUp(1, RmlWin32::GetKeyModifierState());
      break;
    case WM_MBUTTONDOWN:
      result =
          context->ProcessMouseButtonDown(2, RmlWin32::GetKeyModifierState());
      break;
    case WM_MBUTTONUP:
      result =
          context->ProcessMouseButtonUp(2, RmlWin32::GetKeyModifierState());
      break;
    case WM_MOUSEMOVE:
      result =
          context->ProcessMouseMove(static_cast<int>((short)LOWORD(l_param)),
                                    static_cast<int>((short)HIWORD(l_param)),
    ...
    case WM_MOUSEWHEEL:
      result = context->ProcessMouseWheel(
          static_cast<float>((short)HIWORD(w_param)) /
    ...
    case WM_MOUSELEAVE:
      ...
      break;
    case WM_KEYDOWN:
      result = context->ProcessKeyDown(RmlWin32::ConvertKey((int)w_param),
                                       RmlWin32::GetKeyModifierState());
      break;
    case WM_KEYUP:
      result = context->ProcessKeyUp(RmlWin32::ConvertKey((int)w_param),
                                     RmlWin32::GetKeyModifierState());
      break;
    case WM_CHAR: {
      ...
      if (c >= 0xD800 && c < 0xDC00) {
        ...
        if (c >= 0xDC00 && c < 0xE000 && first_u16_code_unit != 0) {
          ...
          Rml::String utf8 =
              ConvertToUTF8(std::wstring{first_u16_code_unit, c});
          character = Rml::StringUtilities::ToCharacter(
    ...
    case WM_IME_STARTCOMPOSITION:
      ...
      // Prevent the native composition window from appearing by capturing the
      // message.
      result = false;
      break;
    ...
    case WM_IME_COMPOSITION: {
      ...
      if (!!(l_param & GCS_CURSORPOS)) {
        // The cursor position is the wchar_t offset in the composition string.
        // Because we work with UTF-8 and not UTF-16, we will have to convert
        // the character offset.
        int cursor_pos = IMEGetCursorPosition(imm_context);

        std::wstring composition = IMEGetCompositionString(imm_context, false);
        Rml::String converted =
            RmlWin32::ConvertToUTF8(composition.substr(0, cursor_pos));
      ...
      if (!!(l_param & CS_NOMOVECARET)) {
        // Suppress the cursor position update. CS_NOMOVECARET is always a part
        // of a more complex message which means that the cursor is updated from
        // a different event.
      ...
      if (!!(l_param & GCS_RESULTSTR)) {
        ...
        text_input_method_editor.ConfirmComposition(
            RmlWin32::ConvertToUTF8(composition));
      }

      if (!!(l_param & GCS_COMPSTR)) {
        std::wstring composition = IMEGetCompositionString(imm_context, false);
        text_input_method_editor.SetComposition(
            RmlWin32::ConvertToUTF8(composition));
      }

      // The composition has been canceled.
      if (!l_param) text_input_method_editor.CancelComposition();

      ImmReleaseContext(window_handle, imm_context);
    } break;
    case WM_IME_CHAR:
    case WM_IME_REQUEST:
      // Ignore WM_IME_CHAR and WM_IME_REQUEST to block the system from
      // appending the composition string.
      result = false;
      break;
    default:
      break;
...
void TextInputMethodEditor_Win32::OnActivate(
    Rml::TextInputContext* _input_context) {
  input_context = _input_context;
}

void TextInputMethodEditor_Win32::OnDeactivate(
    Rml::TextInputContext* _input_context) {
  if (input_context == _input_context) input_context = nullptr;
}

void TextInputMethodEditor_Win32::OnDestroy(
    Rml::TextInputContext* _input_context) {
  if (input_context == _input_context) input_context = nullptr;
}

bool TextInputMethodEditor_Win32::IsComposing() const { return composing; }
...
void TextInputMethodEditor_Win32::EndComposition() {
  if (input_context != nullptr) input_context->SetCompositionRange(0, 0);
...
void TextInputMethodEditor_Win32::CancelComposition() {
  ...
  if (input_context != nullptr) {
    ...
    input_context->SetText(Rml::StringView(), composition_range_start,
...
void TextInputMethodEditor_Win32::SetComposition(Rml::StringView composition) {
  ...
  // Update the composition range only if the cursor can be moved around.
  // Editors working with a single character (e.g., Hangul IME) should have no
  // visual feedback; they use a selection range instead.
  if (cursor_pos != -1 && input_context != nullptr)
    input_context->SetCompositionRange(composition_range_start,
                                       composition_range_end);
}

void TextInputMethodEditor_Win32::ConfirmComposition(
...
void TextInputMethodEditor_Win32::SetCursorPosition(int _cursor_pos,
                                                    bool update) {
  RMLUI_ASSERT(IsComposing());

  cursor_pos = _cursor_pos;

  if (update) UpdateCursorPosition();
}

void TextInputMethodEditor_Win32::SetCompositionString(
    Rml::StringView composition) {
  if (input_context == nullptr) return;

  // Retrieve the composition range if it is missing.
  if (composition_range_start == 0 && composition_range_end == 0)
    input_context->GetSelectionRange(composition_range_start,
                                     composition_range_end);

  input_context->SetText(composition, composition_range_start,
                         composition_range_end);
...
void TextInputMethodEditor_Win32::UpdateCursorPosition() {
  // Cursor position update happens before a composition is set; ignore this
  // event.
  if (input_context == nullptr ||
      (composition_range_start == 0 && composition_range_end == 0))
    return;
  ...
  if (cursor_pos != -1) {
    ...
    // If the API reports no cursor position, select the entire composition
    // string for a better UX.
    input_context->SetSelectionRange(composition_range_start,
                                     composition_range_end);
  }
}