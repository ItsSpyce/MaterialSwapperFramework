...
NK_API void nk_d3d11_render(ID3D11DeviceContext *context,
                            nk_anti_aliasing AA) {
  ...
  { /* load draw vertices & elements directly into vertex + element buffer */
    ...
    hr = context->Map(d3d11.vertex_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                      &vertices);
    NK_ASSERT(SUCCEEDED(hr));
    hr = context->Map(d3d11.index_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                      &indices);
    NK_ASSERT(SUCCEEDED(hr));
    ...
    { /* fill converting configuration */
      ...
      NK_STORAGE const nk_draw_vertex_layout_element vertex_layout[] = {
          {NK_VERTEX_POSITION, NK_FORMAT_FLOAT,
           NK_OFFSETOF(struct nk_d3d11_vertex, position)},
          {NK_VERTEX_TEXCOORD, NK_FORMAT_FLOAT,
           NK_OFFSETOF(struct nk_d3d11_vertex, uv)},
          {NK_VERTEX_COLOR, NK_FORMAT_R8G8B8A8,
           NK_OFFSETOF(struct nk_d3d11_vertex, col)},
          {NK_VERTEX_LAYOUT_END}};
      memset(&config, 0, sizeof(config));
      config.vertex_layout = vertex_layout;
      config.vertex_size = sizeof(nk_d3d11_vertex);
      config.vertex_alignment = NK_ALIGNOF(nk_d3d11_vertex);
      ...
      { /* setup buffers to load vertices and elements */
        nk_buffer vbuf, ibuf;
        nk_buffer_init_fixed(&vbuf, vertices.pData,
                             (size_t)d3d11.max_vertex_buffer);
        nk_buffer_init_fixed(&ibuf, indices.pData,
                             (size_t)d3d11.max_index_buffer);
        nk_convert(&d3d11.ctx, &d3d11.cmds, &vbuf, &ibuf, &config);
      }
    }

    context->Unmap(d3d11.vertex_buffer, 0);
    context->Unmap(d3d11.index_buffer, 0);

    /* iterate over and execute each draw command */
    nk_draw_foreach(cmd, &d3d11.ctx, &d3d11.cmds) {
      D3D11_RECT scissor;
      ID3D11ShaderResourceView *texture_view =
          (ID3D11ShaderResourceView *)cmd->texture.ptr;
      ...
      context->DrawIndexed(cmd->elem_count, offset, 0);
...
static void nk_d3d11_get_projection_matrix(int width, int height,
                                           float *result) {
  const float L = 0.0f;
...
NK_API void nk_d3d11_resize(ID3D11DeviceContext *context, int width,
                            int height) {
  D3D11_MAPPED_SUBRESOURCE mapped;
  if (SUCCEEDED(context->Map(d3d11.const_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                             &mapped))) {
    nk_d3d11_get_projection_matrix(width, height, (float *)mapped.pData);
    context->Unmap(d3d11.const_buffer, 0);
...
NK_API int nk_d3d11_handle_event(HWND wnd, UINT msg, WPARAM wparam,
                                 LPARAM lparam) {
  switch (msg) {
    case WM_KEYDOWN:
    case WM_KEYUP:
    ...
    case WM_LBUTTONDOWN:
      nk_input_button(&d3d11.ctx, NK_BUTTON_LEFT, (short)LOWORD(lparam),
                      (short)HIWORD(lparam), 1);
      SetCapture(wnd);
      return 1;

    case WM_LBUTTONUP:
      nk_input_button(&d3d11.ctx, NK_BUTTON_DOUBLE, (short)LOWORD(lparam),
    ...
    case WM_RBUTTONDOWN:
      nk_input_button(&d3d11.ctx, NK_BUTTON_RIGHT, (short)LOWORD(lparam),
                      (short)HIWORD(lparam), 1);
      SetCapture(wnd);
      return 1;
    ...
    case WM_MBUTTONDOWN:
      nk_input_button(&d3d11.ctx, NK_BUTTON_MIDDLE, (short)LOWORD(lparam),
                      (short)HIWORD(lparam), 1);
      SetCapture(wnd);
      return 1;
...
static void nk_d3d11_clipboard_paste(nk_handle usr, nk_text_edit *edit) {
  (void)usr;
      ...
      if (size) {
        ...
        if (wstr) {
          int utf8size = WideCharToMultiByte(
              CP_UTF8, 0, wstr, size / sizeof(wchar_t), NULL, 0, NULL, NULL);
          if (utf8size) {
            char *utf8 = (char *)malloc(utf8size);
            if (utf8) {
              WideCharToMultiByte(CP_UTF8, 0, wstr, size / sizeof(wchar_t),
...
NK_API nk_context *nk_d3d11_init(ID3D11Device *device, int width,
                                 int height,
                                 unsigned int max_vertex_buffer,
                                 unsigned int max_index_buffer) {
  HRESULT hr;
  ...
  /* vertex shader */
  {
    hr = device->CreateVertexShader(nk_d3d11_vertex_shader,
                                    sizeof(nk_d3d11_vertex_shader), NULL,
                                    &d3d11.vertex_shader);
    NK_ASSERT(SUCCEEDED(hr));
  }
  ...
  /* input layout */
  {
    const D3D11_INPUT_ELEMENT_DESC layout[] = {
        {"POSITION", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0,
         offsetof(struct nk_d3d11_vertex, position),
         D3D11_INPUT_PER_VERTEX_DATA, 0},
        {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
         offsetof(struct nk_d3d11_vertex, uv), D3D11_INPUT_PER_VERTEX_DATA, 0},
        {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
         offsetof(struct nk_d3d11_vertex, col), D3D11_INPUT_PER_VERTEX_DATA, 0},
    };
    hr = device->CreateInputLayout(
        layout, ARRAYSIZE(layout), nk_d3d11_vertex_shader,
        sizeof(nk_d3d11_vertex_shader), &d3d11.input_layout);
    NK_ASSERT(SUCCEEDED(hr));
  }
  ...
  /* pixel shader */
  {
    hr = device->CreatePixelShader(nk_d3d11_pixel_shader,
                                   sizeof(nk_d3d11_pixel_shader), NULL,
                                   &d3d11.pixel_shader);
    NK_ASSERT(SUCCEEDED(hr));
  }
...
NK_API void nk_d3d11_font_stash_begin(nk_font_atlas **atlas) {
...
NK_API void nk_d3d11_font_stash_end(void) {
  ...
  /* upload font to texture and create texture view */
  {
...

    {
      ...
      hr = d3d11.device->CreateShaderResourceView(font_texture, &srv,
                                                  &d3d11.font_texture_view);
      assert(SUCCEEDED(hr));
    }
    font_texture->Release();
  }

  nk_font_atlas_end(&d3d11.atlas, nk_handle_ptr(d3d11.font_texture_view),
                    &d3d11.tex_null);
...
void nk_d3d11_shutdown(void) {
  ...
  if (d3d11.font_texture_view) d3d11.font_texture_view->Release();
  if (d3d11.vertex_buffer) d3d11.vertex_buffer->Release();
  if (d3d11.index_buffer) d3d11.index_buffer->Release();
  if (d3d11.blend_state) d3d11.blend_state->Release();
  if (d3d11.pixel_shader) d3d11.pixel_shader->Release();
  if (d3d11.const_buffer) d3d11.const_buffer->Release();
  if (d3d11.vertex_shader) d3d11.vertex_shader->Release();
  if (d3d11.input_layout) d3d11.input_layout->Release();
  if (d3d11.rasterizer_state) d3d11.rasterizer_state->Release();
