...
bool Condition::Evaluate(RE::TESObjectREFR* refr) const {
  string type;
  ranges::transform(this->type, type.begin(), [](unsigned const char c) {
  ...
  if (type == "morph") {
    ...
    _DEBUG("Key: {}, morph: {}, comparator: {}, comparison: {}", keyName,
           morphName, comparator, comparison);
