...
HRESULT CompileShaderFromData(LPCVOID pSrcData, _In_ SIZE_T SrcDataSize, _In_opt_ LPCSTR pSourceName, _In_ LPCSTR entryPoint, _In_ LPCSTR profile, _Outptr_ ID3DBlob** blob, _Outptr_ ID3DBlob** errorBlob)
{
  ...
  constexpr D3D_SHADER_MACRO defines[] =
	...
	for (UINT32 i = 0; i < sizeof(versions) / sizeof(const char*); ++i)
	{
		...
		d3dcompiler = LoadLibrary(reinterpret_cast<LPCWSTR>(name));
		if (d3dcompiler)
			break;
	}

	if (!d3dcompiler) {
		logger::error("{} - Failed to find d3dcompiler module", __FUNCTION__);
		return E_NOINTERFACE;
	}

	auto compile = (_D3DCompile)GetProcAddress(d3dcompiler, "D3DCompile");
	if (!compile) {
		logger::error("{} - Failed to find D3DCompile function", __FUNCTION__);
		return E_NOINTERFACE;
	}
